// PlayerTri4To1SubPanel.cs
// Tri4To1Tool の Player 版サブパネル（UIToolkit）。
...
namespace Poly_Ling.Player
{
    public class PlayerTri4To1SubPanel
    {
        ...
        public void Build(VisualElement parent)
        {
            ...
            _root.Add(Header("Triangle 4 to 1 / 三角形4→1統合"));
            _root.Add(new HelpBox(
                "選択した三角形と、それを囲む三角形3枚を消して、外側の3頂点を結ぶ三角形1枚に張り替えます。\n" +
                "中点細分割の逆操作にあたります。\n" +
                "囲む面に三角形でないものがある、または境界の辺があるときは対象外です。\n" +
                "選択面の3頂点は、どの面からも使われなくなったものだけ消えます。\n" +
                "複数オブジェクト・複数面に対応。4枚組が重なる面どうしは干渉するため除外します。",
            ...
            {
                SendCommand?.Invoke(new Tri4To1Command(ModelIndex, SelectedMasterIndices()));
                Refresh();
            })
            { text = "三角形4→1 実行" };
        ...
        private void UpdateStats()
        {
            ...
            if (!info.Item("canExecute", false))
            {
                if (_targetLabel != null)
                    _targetLabel.text = $"選択中: {Surface.GetInt(Tool, "selectedFaceCount")} 面  /  除外: {skipped} 面";
            ...
            if (_targetLabel != null)
                _targetLabel.text = $"対象: {info.Item("objectCount", 0)} オブジェクト / {info.Item("targetCount", 0)} 面"
