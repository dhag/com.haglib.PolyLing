// FaceMergeCollapseToolHandler.cs
// FaceMergeCollapseTool を Player の入力イベントに橋渡しする IPlayerToolHandler 実装。
...
{
    public class FaceMergeCollapseToolHandler : IPlayerToolHandler
    {
        // ================================================================
        // 依存
        // ================================================================

        private readonly FaceMergeCollapseTool  _tool = new FaceMergeCollapseTool();
        ...
        public FaceMergeCollapseTool.MergeSummary Inspect() => _tool.Inspect();

        /// <summary>
        /// 面の結合（頂点を外す方式）を実行する。
        ///
        /// private にしてある。パネル・ショートカットからの直呼びは塞ぎ、
        /// FaceMergeCollapseCommand 経由に統一するため。
        /// </summary>
        private void TriggerMergeCore() => _tool.TriggerMerge();

        /// <summary>
        /// 面の結合（頂点を外す方式）コマンドを実行する。
        ///
        /// 【マウス／パネル経路と同じ実装を通す】
        ///   実処理は FaceMergeCollapseTool が正典。ここは対象の照合だけを行い、
        ...
        public bool ExecuteFromCommand(Poly_Ling.Data.FaceMergeCollapseCommand cmd, out string reason)
        {
            reason = null;
