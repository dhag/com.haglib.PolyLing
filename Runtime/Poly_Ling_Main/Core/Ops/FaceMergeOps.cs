...
//   4. 共有頂点 a, b のうち、結合する2枚以外のどの面からも使われていないものは
//      多角形の並びから外し、頂点自体も削除する（前後の点が直接つながる）。
//      ただし外した結果 3頂点未満になる場合は、どちらも外さない。
//      例: 三角形同士 → 四角形（両端点が浮いていても外さない）
//          三角形と四角形 → 三角形 / 四角形同士 → 四角形（両端点が浮いていれば外す）
//
// 【不変条件（厳守）】
...
{
    public static class FaceMergeOps
    {
        /// <summary>実行可否と結合結果の規模。パネルの表示・ボタン活性に使う。</summary>
        public struct FaceMergeInfo
        {
        ...
        public static FaceMergeInfo Inspect(MeshObject mo, VertexPair edge)
        {
            var info = new FaceMergeInfo();
            ...
            if (!BuildMerge(mo, edge, out _, out _, out var ring, out _, out _,
                            out var killed, out string reason))
            {
                info.Reason = reason;
                return info;
            }
            ...
            info.RemovedVertexCount = killed.Count;
            ...
            out string reason)
        {
            ...
            // --- 結合する2枚以外から使われていない共有頂点を洗い出す ---
            var used = new HashSet<int>();
            for (int fi = 0; fi < mo.Faces.Count; fi++)
            {
                if (fi == faceIndexA || fi == faceIndexB) continue;
                foreach (int v in mo.Faces[fi].VertexIndices) used.Add(v);
            }

            var removable = new List<int>();
            if (!used.Contains(a)) removable.Add(a);
            if (!used.Contains(b)) removable.Add(b);

            // 全部外すと3頂点未満になる場合は、どちらも外さない。
            if (removable.Count > 0 && ring.Count - removable.Count >= 3)
            {
                var kill = new HashSet<int>(removable);
                ...
                killedVertices.AddRange(removable);
                killedVertices.Sort();
            }
            ...
            out int resultVertexCount, out List<int> removedVertices, out string reason)
        {
            ...
            // どの面からも参照されなくなった共有頂点を削除し、インデックスを詰める。
            if (killed.Count > 0)
            {
        ...
        public struct FaceMergeBatchInfo
        {
            /// <summary>実行できるか。</summary>
            public bool CanExecute;
        ...
        public static FaceMergeBatchInfo InspectMany(MeshObject mo, IEnumerable<VertexPair> edges)
        {
            var info = new FaceMergeBatchInfo();
