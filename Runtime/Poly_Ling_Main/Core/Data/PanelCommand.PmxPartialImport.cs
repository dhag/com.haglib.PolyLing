...
namespace Poly_Ling.Data
{
    ...
    [PLCommand(Category = "io.import", Writes = PLWriteScope.None, Description = "PMX ファイルを読んで、取り込めるオブジェクトを一覧する。モデルは変えない。")]
    ...
    [PLCommand(Category = "io.import", Writes = PLWriteScope.None, Description = "名前の完全一致を先に見て、外れたら頂点数の一致で、モデルの描画オブジェクトと PMX のオブジェクトを組む。組を返すだけでモデルは変えない。返った組を必ず見ること。")]
    ...
    [PLCommand(Category = "io.import", Writes = PLWriteScope.ModelWide, Description = "指定した組で、PMX の頂点位置・UV・ボーンウェイトをモデルの描画オブジェクトへ差し替える。頂点数も索引も変わらない。組は modelIndices と pmxIndices の同じ位置どうし。")]
