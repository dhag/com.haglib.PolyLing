...
namespace Poly_Ling.Data
{
    ...
    [PLCommand(Category = "io.import", Writes = PLWriteScope.None, Description = "MQO ファイルを読んで、取り込める（可視・頂点あり）オブジェクトを一覧する。モデルは変えない。")]
    ...
    [PLCommand(Category = "io.import", Writes = PLWriteScope.None, Description = "展開頂点数の一致で、モデルの描画オブジェクトと MQO のオブジェクトを組む。組を返すだけでモデルは変えない。同じ頂点数のものが複数あると別物と組むので、返った組を必ず見ること。")]
    ...
    [PLCommand(Category = "io.import", Writes = PLWriteScope.ModelWide, Description = "指定した組で、MQO の頂点位置（と UV）をモデルの描画オブジェクトへ差し替える。頂点数も索引も変わらない。組は modelIndices と mqoIndices の同じ位置どうし。")]
