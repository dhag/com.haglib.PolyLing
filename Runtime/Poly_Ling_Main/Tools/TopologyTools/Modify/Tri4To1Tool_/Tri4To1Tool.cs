// Tri4To1Tool.cs
// 三角形4→1統合ツール - 選択した三角形と、それを囲む三角形3枚を1枚に統合する。
// 実処理は Tri4To1Ops。ここは対象の集約・Undo 記録・通知だけを担う。
//
// 【対象】選択中の描画オブジェクト全部（マルチセレクト対応）× 各オブジェクトの
//   選択面全部（複数対応）。4枚組が重なる面は Tri4To1Ops 側で除外される。
//
// 【トポロジカル変更の分類】
...
//   （メッシュが何個でも Undo は1手）。VertexHoleTool / VertexDissolveTool と同じ方針。
...
{
    /// <summary>
    /// 三角形4→1統合ツール。マウス操作は持たず、UI からの実行のみ。
    /// </summary>
    public class Tri4To1Tool : IEditTool
    {
        public string Name        => "Tri4To1";
        public string DisplayName => "Triangle 4 to 1";
        ...
        /// <summary>対象メッシュ全部を合わせた選択面数。</summary>
        public int SelectedFaceCount
        {
            get
            {
                int total = 0;
                foreach (var t in EnumerateTargets()) total += t.Faces.Count;
                return total;
            }
        }
        ...
        /// <summary>
        /// 選択中の描画オブジェクトを走査し、選択面を持つものだけを返す。
        /// ボーン表示用メッシュは編集対象外。
        /// </summary>
        ...
        /// <summary>
        /// 対象の下調べ。選択面が無い・全部除外されるときは CanExecute=false。
        /// </summary>
        public MergeSummary Inspect()
        {
            ...
            if (targets.Count == 0)
            {
                sum.Reason = "面を選択してください";
                return sum;
            }

            foreach (var t in targets)
            {
                var info = Tri4To1Ops.InspectMany(t.MeshContext.MeshObject, t.Faces);
        ...
        /// <summary>統合を実行する。対象メッシュすべてを1回の Undo にまとめる。</summary>
        public void TriggerMerge()
        {
            ...
            {
                Debug.LogWarning($"[Tri4To1Tool] 実行中止: model={model != null}, targets={targets.Count}");
                return;
            }
            ...
            foreach (var t in targets) opInputs[t.MeshIndex] = t.Faces;

            foreach (var t in targets)
            {
                bool ok = Tri4To1Ops.ExecuteMany(
                    t.MeshContext.MeshObject, t.Faces,
            ...
            if (okMeshes == 0)
            {
                Debug.LogWarning($"[Tri4To1Tool] 実行失敗: {lastReason ?? "対象がありません"}");
                return;
            }
            ...
            int mirrorApplied = MirrorBranchOps.ApplyToMirrors(model, mirrorPlan, (realIdx, mirrorMo) =>
            {
                ...
                return Tri4To1Ops.ExecuteMany(mirrorMo, src,
            ...
            if (undo != null)
            {
                ...
                string desc = $"Triangle 4 to 1 ({okMeshes} objs / {mergedTotal} patches / {removedFaceTotal} faces)";
            ...
            Debug.Log($"[Tri4To1Tool] 統合完了: オブジェクト {okMeshes} / 統合 {mergedTotal} 箇所 "
