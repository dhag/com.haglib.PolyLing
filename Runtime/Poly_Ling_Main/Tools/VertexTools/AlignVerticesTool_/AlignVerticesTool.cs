...
namespace Poly_Ling.Tools
{
    ...
    public partial class AlignVerticesTool : IEditTool
    {
        ...
        private void CalculateAndAutoSelect(ToolContext ctx)
        {
            ...
            foreach (int idx in ctx.SelectedVertices)
            {
                if (idx >= 0 && idx < ctx.ActiveMeshObject.VertexCount)
                    positions.Add(ctx.ActiveMeshObject.Vertices[idx].Position);
            }
        ...
        private Vector3 CalculateAlignTarget()
        {
            ...
            {
                if (idx >= 0 && idx < _context.ActiveMeshObject.VertexCount)
                    positions.Add(_context.ActiveMeshObject.Vertices[idx].Position);
            }
        ...
        private void ExecuteAlign()
        {
            ...
            foreach (int idx in _context.SelectedVertices)
            {
                if (idx < 0 || idx >= _context.ActiveMeshObject.VertexCount) continue;

                Vertex  v      = _context.ActiveMeshObject.Vertices[idx];
            ...
            if (movedCount > 0)
            {
                _context.ActiveMeshObject.InvalidatePositionCache();
