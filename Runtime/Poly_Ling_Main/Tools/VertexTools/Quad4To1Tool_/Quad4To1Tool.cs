// Quad4To1Tool.cs
// 四角形4→1統合ツール - 選択した頂点を共有する四角形4枚を、四隅の四角形1枚に統合する。
// 実処理は Quad4To1Ops。ここは対象の集約・Undo 記録・通知だけを担う。
//
// 【対象】選択中の描画オブジェクト全部（マルチセレクト対応）× 各オブジェクトの
//   選択頂点全部（複数対応）。同じ面を共有する頂点は Quad4To1Ops 側で除外される。
//
// 【トポロジカル変更の分類】
...
{
    /// <summary>
    /// 四角形4→1統合ツール。マウス操作は持たず、UI からの実行のみ。
    /// </summary>
    public class Quad4To1Tool : IEditTool
    {
        public string Name        => "Quad4To1";
        public string DisplayName => "Quad 4 to 1";
        ...
        public MergeSummary Inspect()
        {
            ...
            {
                var info = Quad4To1Ops.InspectMany(t.MeshContext.MeshObject, t.Apexes);
        ...
        public void TriggerMerge()
        {
            ...
            {
                Debug.LogWarning($"[Quad4To1Tool] 実行中止: model={model != null}, targets={targets.Count}");
                return;
            }
            ...
            foreach (var t in targets)
            {
                bool ok = Quad4To1Ops.ExecuteMany(
            ...
            if (okMeshes == 0)
            {
                Debug.LogWarning($"[Quad4To1Tool] 実行失敗: {lastReason ?? "対象がありません"}");
                return;
            }
            ...
            int mirrorApplied = MirrorBranchOps.ApplyToMirrors(model, mirrorPlan, (realIdx, mirrorMo) =>
            {
                ...
                return Quad4To1Ops.ExecuteMany(mirrorMo, src,
            ...
            if (undo != null)
            {
                ...
                string desc = $"Quad 4 to 1 ({okMeshes} objs / {mergedTotal} patches / {removedFaceTotal} faces)";
            ...
            Debug.Log($"[Quad4To1Tool] 統合完了: オブジェクト {okMeshes} / 統合 {mergedTotal} 箇所 "
