// McpCylinderMeshGenerator.cs
// MCP用サンドボックスの円筒生成ロジック。
// Runtime/Poly_Ling_Main/Tools/PrimitiveMesh/ に配置
//
// 【なぜ CylinderMeshGenerator の複製なのか】
//   サンドボックスは形状の数式をいじって試すための場所で、いじった結果が
//   「基本図形」の円柱へ波及してはいけない。共有すると片方の実験がもう片方を壊す。
//   出発点は CylinderMeshGenerator と同一で、以後は別々に育てる前提で分けてある。
...
{
    public static class McpCylinderMeshGenerator
    {
        // ================================================================
        // パラメータ構造体
        // ================================================================
        [Serializable]
        public struct McpCylinderParams : IEquatable<McpCylinderParams>
        {
            // ── 値域 ─────────────────────────────────────────────────
            // PLParam 属性とサンドボックスパネルの行ヘルパの双方がここを参照する。
            ...
            public static McpCylinderParams Default => new McpCylinderParams
            {
                MeshName       = "McpCylinder",
            ...
            public bool Equals(McpCylinderParams o) =>
                MeshName == o.MeshName &&
            ...
            public override bool Equals(object obj) => obj is McpCylinderParams p && Equals(p);
        ...
        // 生成
        // ================================================================
        public static MeshObject Generate(McpCylinderParams p)
        {
        ...
        private static void GenerateSimple(MeshObject md, McpCylinderParams p, Vector3 pivot)
        {
        ...
        private static void AddCapSimple(MeshObject md, McpCylinderParams p, float y, float radius, bool top, Vector3 pivot)
        {
        ...
        private static void GenerateRounded(MeshObject md, McpCylinderParams p, Vector3 pivot)
        {
            float halfH = p.Height * 0.5f;
