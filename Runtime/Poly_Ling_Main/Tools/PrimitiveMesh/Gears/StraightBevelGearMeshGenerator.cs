// StraightBevelGearMeshGenerator.cs
// すぐばかさ歯車のメッシュ生成。
// Runtime/Poly_Ling_Main/Tools/PrimitiveMesh/Gears/ に配置
//
// 【形状】
//   歯すじがピッチ円錐の母線に沿ってまっすぐ走るかさ歯車。
//   歯形はトレッドゴルドの相当平歯車（インボリュート歯面＋トロコイド歯元）で決め、
//   背円錐の展開を写し戻して外端の断面を作る。あとは小端まで相似に縮めるだけ。
//
// 【相手の歯数と軸角】
//   ピッチ円錐角は自分の歯数だけでは決まらない。相手の歯数と軸角が要る。
//
//       tan(δ) = sin(Σ) / (z2/z1 + cos(Σ))
//
//   軸角 90° で z1=z2 のときが δ=45°（マイタギヤ）。
//
// 【軸の向き】
//   軸は Z、大端が +Z 側、小端が -Z 側。Z の中央そろえで置く。
//   軸穴はまっすぐな円筒で、小端の歯元へ食い込まない大きさに抑える。
//
// 【共有部品】
//   円錐まわりと断面列は Gears/BevelGearSection、歯形は Gears/InvoluteTrochoidSection。
//   まがりばかさ歯車（SpiralBevelGearMeshGenerator）とは、ねじれ角 0 かどうかだけが違う。
...
{
    public static class StraightBevelGearMeshGenerator
    {
        // ================================================================
        // パラメータ構造体
        // ================================================================
        ...
        [System.Serializable]
        public struct StraightBevelGearParams : System.IEquatable<StraightBevelGearParams>
        {
            // ── 値域 ─────────────────────────────────────────────────
            ...
            public static StraightBevelGearParams Default => new StraightBevelGearParams
            {
                MeshName          = "StraightBevelGear",
                ToothCount        = 20,
                MatingToothCount  = 40,
                ShaftAngleDeg     = 90f,
                Module            = 0.1f,
            ...
            public bool Equals(StraightBevelGearParams o) =>
                MeshName == o.MeshName &&
            ...
            public override bool Equals(object obj) => obj is StraightBevelGearParams p && Equals(p);
        ...
        private static bool TryGetBevel(
            StraightBevelGearParams p, out BevelGearSection.BevelData g)
        {
            var input = new BevelGearSection.BevelInput
            {
                ...
                Module              = p.Module,
                NormalPressureAngle = p.PressureAngleDeg * Mathf.Deg2Rad,
        ...
        private static InvoluteTrochoidSection.Samples ToSamples(StraightBevelGearParams p)
        ...
        public static BevelGearSection.BevelInfo GetInfo(StraightBevelGearParams p)
        {
        ...
        /// <summary>
        /// すぐばかさ歯車メッシュを作る。パラメータが不正なときは空メッシュを返す。
        /// </summary>
        public static MeshObject Generate(StraightBevelGearParams p)
        {
            string name = string.IsNullOrEmpty(p.MeshName) ? "StraightBevelGear" : p.MeshName;
