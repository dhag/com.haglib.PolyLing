// Runtime/Poly_Ling_Main/HierarchyIO/HumanoidSupplementBuilder.cs
// ============================================================
// Humanoid 必須関節のダミー補完
// ============================================================
//
...
//
// 【なぜ Runtime に置くか】
//   もとは Editor/AvatarBuilder/ に置いていたが、UnityEditor への依存は無く
//   UnityEngine.HumanTrait しか使っていない。VRM 書き出し（PolyLing.Vrm10）も
//   同じ補完を必要とするが、PolyLing.Vrm10 は PolyLing.Editor を参照できない。
//   両方から呼べるよう Runtime へ移し、HumanoidTransformMap と同じ
//   Poly_Ling.HierarchyIO 名前空間に置く。ロジックは移動前から変えていない。
//
// ============================================================
...
namespace Poly_Ling.HierarchyIO
{
    ...
    public static class HumanoidSupplementBuilder
    {
        ...
        /// 戻り値は補完した関節数。
        /// </summary>
        /// <param name="resolvedOut">
        /// 非 null のとき、補完後の humanName→Transform をここへ書き出す。
        /// 名前ではなく Transform を要る呼び出し（UniHumanoid.Humanoid.AssignBones）のための口。
        /// 既存の割当も含めた全件が入る。
        /// </param>
        public static int Supplement(
            ...
            Action<string> log,
            Action<string> warn,
            Dictionary<string, Transform> resolvedOut = null)
        {
            ...
            {
                resolvedOut.Clear();
                foreach (var kv in resolved) resolvedOut[kv.Key] = kv.Value;
            }
