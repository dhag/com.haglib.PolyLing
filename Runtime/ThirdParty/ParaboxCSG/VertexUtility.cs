...
//   - TransformVertex を Transform 版から Matrix4x4 版へ置換。
//   - SetMesh で頂点数 65535 超のとき IndexFormat.UInt32 を設定。
...
namespace Poly_Ling.CSG
{
	static class VertexUtility
	{
        ...
        public static void SetMesh(Mesh mesh, IList<Vertex> vertices)
        {
            ...
            // PolyLing 追加: CSG の結果は頂点数が膨らみやすい。
            // 16bit インデックスの上限を超える場合は 32bit へ切り替える。
        ...
        /// </summary>
        /// <remarks>
        /// PolyLing 改変: 元は Transform を受ける拡張メソッドだったものを
        /// Matrix4x4 版に置換した。PolyLing の MeshContext は GameObject を
        /// 持たず Matrix4x4 で姿勢を表すため。
        /// 法線は非一様スケールでも正しくなるよう逆転置行列で変換する。
        /// 接線は xyz のみ変換し、w（従法線の向き）はそのまま保つ。
        /// </remarks>
        /// <param name="matrix">The matrix to apply.</param>
        /// <param name="normalMatrix">
        /// 法線用の逆転置行列。matrix.inverse.transpose を呼び出し側で
        /// 一度だけ作って渡す（頂点ごとに計算すると重いため）。
        /// </param>
        /// <param name="vertex">A model space vertex.</param>
        /// <returns>A new vertex in the target coordinate space.</returns>
        public static Vertex TransformVertex(Matrix4x4 matrix, Matrix4x4 normalMatrix, Vertex vertex)
        {
            var v = new Vertex();

            if (vertex.HasArrays(VertexAttributes.Position))
                v.position = matrix.MultiplyPoint3x4(vertex.position);

            if (vertex.HasArrays(VertexAttributes.Color))
                v.color = vertex.color;

            if (vertex.HasArrays(VertexAttributes.Normal))
                v.normal = normalMatrix.MultiplyVector(vertex.normal).normalized;

            if (vertex.HasArrays(VertexAttributes.Tangent))
            {
                Vector3 t3 = matrix.MultiplyVector(new Vector3(
                    vertex.tangent.x, vertex.tangent.y, vertex.tangent.z)).normalized;
                v.tangent = new Vector4(t3.x, t3.y, t3.z, vertex.tangent.w);
            }
