...
namespace MeshFactory.UndoSystem
{
    ...
    {
        // Note: この拡張を使用する場合、MeshUndoController.csで
        // partial classを宣言し、以下のフィールドとメソッドを追加してください。

        /*
        // === 追加フィールド（MeshUndoController.csに追加） ===
        private UndoStack<ExportSettings> _exportSettingsStack;
        private ExportSettings _exportSettings;

        // === 追加プロパティ ===
        public ExportSettings ExportSettings => _exportSettings;
        public UndoStack<ExportSettings> ExportSettingsStack => _exportSettingsStack;

        // === コンストラクタ内に追加（_workPlaneStackの後） ===
        // ExportSettingsスタック
        _exportSettings = new ExportSettings();
        _exportSettingsStack = new UndoStack<ExportSettings>(
            $"{windowId}/ExportSettings",
            "Export Settings",
            _exportSettings
        );
        _mainGroup.AddChild(_exportSettingsStack);

        // イベント購読に追加
        _exportSettingsStack.OnUndoPerformed += _ => OnUndoRedoPerformed?.Invoke();
        _exportSettingsStack.OnRedoPerformed += _ => OnUndoRedoPerformed?.Invoke();
        */
    }
