...
//
// 【ビルド版と共通の処理】
//   ping / tools* / scenes / call は Runtime の PolyLingControlProtocol に移した。
//   ビルド版の受信サーバ（PolyLingPlayerControlServer、ポート 8772）も同じものを使う。
//
...
namespace Poly_Ling.EditorControl
{
    ...
    public static class PolyLingEditorControlServer
    {
        ...
        private static string Handle(string requestJson, out Action afterReply, out string action)
        {
            ...
            // ビルド版と共通の action（PolyLingControlProtocol）。
            if (PolyLingControlProtocol.TryHandleCommon(op, msg, LogTag, out string common))
                return LogResponse(common);

            switch (op)
            {
                case "state":
                    // host= で接続先が Editor かビルド版かを見分けられるようにする。
                    return LogResponse(BuildOk(op, "host=editor; " + DescribeState()));
        ...
        // ================================================================
        //
        // ping / tools / tools_generic / tools_search / tools_describe / scenes / call は
        // ビルド版の受信サーバ（PolyLingPlayerControlServer）と共通なので、
        // Runtime の PolyLingControlProtocol で処理する。中身は以前ここにあったものと同じ。
        // ================================================================
        // Play Mode 制御
        // ================================================================
        ...
        private static string BuildOk(string action, string data)
            => PolyLingControlProtocol.BuildOk(action, data);
        ...
        private static string BuildError(string action, string error)
            => PolyLingControlProtocol.BuildError(action, error);
    }
}
