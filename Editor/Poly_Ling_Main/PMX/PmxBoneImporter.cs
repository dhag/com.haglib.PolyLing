...
namespace Poly_Ling.PMX
{
    ...
    public static class PmxBoneImporter
    {
        ...
        public sealed class CsvBone
        {
            ...
            /// <summary>ローカル軸が自動計算されたかどうか</summary>
            ...
            AxisMap map)
        {
            ...
            else
            {
                ...
                // 自動計算結果を記録
                bone.LocalAxisX = localX.normalized;
                bone.LocalAxisZ = localZ;
                bone.IsLocalAxisAutoCalculated = true;
            }
            ...
            // 座標系変換（右手系→左手系）: 共役変換 R_unity = S * R_rh * S
            // X,Y軸: MapDirection（Z成分反転） / Z軸: XY成分反転、Z成分そのまま
            localX = map.MapDirection(localX).normalized;
            localY = map.MapDirection(localY).normalized;
            if (map.flipZ)
                localZ = new Vector3(-localZ.x, -localZ.y, localZ.z).normalized;
            else
        ...
        public static void DebugPrintBoneAxes(IEnumerable<CsvBone> bones, AxisMap map)
        {
            ...
            foreach (var bone in bones)
            {
                ...
                x = map.MapDirection(x);
                y = map.MapDirection(y);
                if (map.flipZ)
                ...
                Debug.Log($"  LocalY: ({y.x:F4}, {y.y:F4}, {y.z:F4})");
