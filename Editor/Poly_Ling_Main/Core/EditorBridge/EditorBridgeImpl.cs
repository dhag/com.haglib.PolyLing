...
namespace Poly_Ling.EditorBridge
{
    ...
    public class EditorBridgeImpl : IEditorBridge
    {
        ...
        {
            var result = new System.Collections.Generic.List<IToolContextReceiver>();
            foreach (var w in Resources.FindObjectsOfTypeAll<EditorWindow>())
                if (w is IToolContextReceiver r) result.Add(r);
            return result.ToArray();
        }

        public IPanelContextReceiver[] FindAllPanelContextReceivers()
        {
            var result = new System.Collections.Generic.List<IPanelContextReceiver>();
            foreach (var w in Resources.FindObjectsOfTypeAll<EditorWindow>())
                if (w is IPanelContextReceiver r) result.Add(r);
            return result.ToArray();
        }
