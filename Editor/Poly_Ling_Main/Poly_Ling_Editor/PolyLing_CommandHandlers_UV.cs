// PolyLing_CommandHandlers_UV.cs
// UV系コマンドハンドラ（UV展開・UVZ変換）
...
public partial class PolyLing
{
    ...
    private void HandleApplyUvUnwrap(ApplyUvUnwrapCommand cmd)
    {
        if (_model == null) return;

        // 先頭メッシュのみUndoを記録（_undoControllerは現選択の1コンテキストのみ対応）
        bool firstDone = false;
        MeshObjectSnapshot before = null;

        foreach (int masterIdx in cmd.MasterIndices)
        {
            var ctx = _model.GetMeshContext(masterIdx);
    ...
    private void HandleUvToXyz(UvToXyzCommand cmd)
    {
        if (_model == null) return;

        var srcCtx = _model.GetMeshContext(cmd.MasterIndex);
    ...
    private void HandleXyzToUv(XyzToUvCommand cmd)
    {
        if (_model == null) return;

        var srcCtx    = _model.GetMeshContext(cmd.SourceMasterIndex);
        var targetCtx = _model.GetMeshContext(cmd.TargetMasterIndex);
        var srcMeshObj    = srcCtx?.MeshObject;
        ...
        {
            var after = _undoController.CaptureMeshObjectSnapshot();
            _undoController.RecordTopologyChange(before, after, "XYZ→UV書き戻し");
        }
