    ...
    public partial class KnifeTool
    {
        ...
        private int FindFrontmostFaceAtPosition(ToolContext ctx, Vector2 screenPos)
        {
            ...
            for (int faceIdx = 0; faceIdx < ctx.FirstSelectedMeshObject.FaceCount; faceIdx++)
            {
                ...
                {
                    var worldPos = ctx.FirstSelectedMeshObject.Vertices[vIdx].Position;
        ...
        private void UpdateIntersections(ToolContext ctx)
        {
            ...
            if (ctx.FirstSelectedMeshObject == null || ctx.FirstSelectedMeshObject.FaceCount == 0) return;
        ...
        private List<EdgeIntersection> FindIntersectionsForFace(ToolContext ctx, Face face)
        {
            ...
            for (int i = 0; i < face.VertexCount; i++)
            {
                ...
                var p0 = ctx.FirstSelectedMeshObject.Vertices[vIdx0].Position;
                var p1 = ctx.FirstSelectedMeshObject.Vertices[vIdx1].Position;
        ...
        private void ExecuteCut(ToolContext ctx)
        {
            ...
            if (_targetFaceIndex < 0 || _targetFaceIndex >= ctx.FirstSelectedMeshObject.FaceCount) return;
            ...
            int newVertexIdx0 = ctx.FirstSelectedMeshObject.VertexCount;
            var newVertex0 = new Vertex(inter0.WorldPos);
            InterpolateVertexAttributes(ctx.FirstSelectedMeshObject, face, inter0.EdgeStartIndex, inter0.EdgeEndIndex, inter0.T, newVertex0);
            ctx.FirstSelectedMeshObject.Vertices.Add(newVertex0);
            addedVertices.Add((newVertexIdx0, newVertex0.Clone()));

            int newVertexIdx1 = ctx.FirstSelectedMeshObject.VertexCount;
            var newVertex1 = new Vertex(inter1.WorldPos);
            InterpolateVertexAttributes(ctx.FirstSelectedMeshObject, face, inter1.EdgeStartIndex, inter1.EdgeEndIndex, inter1.T, newVertex1);
            ctx.FirstSelectedMeshObject.Vertices.Add(newVertex1);
            addedVertices.Add((newVertexIdx1, newVertex1.Clone()));

            var (face1, face2) = SplitFace(face, inter0, inter1, newVertexIdx0, newVertexIdx1);

            ctx.FirstSelectedMeshObject.Faces[_targetFaceIndex] = face1;
            int newFaceIdx = ctx.FirstSelectedMeshObject.FaceCount;
            ctx.FirstSelectedMeshObject.Faces.Add(face2);
